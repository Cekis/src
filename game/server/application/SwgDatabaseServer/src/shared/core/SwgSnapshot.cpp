...
SwgSnapshot::SwgSnapshot(DB::ModeQuery::Mode mode, bool useGoldDatabase) :
		Snapshot(mode, useGoldDatabase),
		m_battlefieldMarkerObjectBuffer(mode),
		m_battlefieldParticipantBuffer(mode),
		m_bountyHunterTargetBuffer(),
		m_buildingObjectBuffer(mode),
		m_cellObjectBuffer(mode),
		m_cityObjectBuffer(mode),
		m_creatureObjectBuffer(mode),
		m_experienceBuffer(mode),
		m_factoryObjectBuffer(mode),
		m_guildObjectBuffer(mode),
		m_harvesterInstallationObjectBuffer(mode),
		m_installationObjectBuffer(mode),
		m_intangibleObjectBuffer(mode),
		m_locationBuffer(mode),
		m_manufactureInstallationObjectBuffer(mode),
		m_manufactureSchematicAttributeBuffer(mode),
		m_manufactureSchematicObjectBuffer(mode),
		m_messageBuffer(),
		m_missionObjectBuffer(mode),
		m_objectTableBuffer(mode),
		m_objvarBuffer(mode, &m_objectTableBuffer, useGoldDatabase),
		m_planetObjectBuffer(mode),
		m_playerObjectBuffer(mode),
		m_playerQuestObjectBuffer(mode),
		m_propertyListBuffer(mode),
		m_resourceContainerObjectBuffer(mode),
		m_resourceTypeBuffer(),
		m_scriptBuffer(),
		m_shipObjectBuffer(mode),
		m_staticObjectBuffer(mode),
		m_tangibleObjectBuffer(mode),
		m_universeObjectBuffer(mode),
...
void SwgSnapshot::handleDeleteMessage(const NetworkId &objectId, int reasonCode, bool immediate, bool demandLoadedContainer, bool cascadeReason)
{
...
void SwgSnapshot::decodeAttributes(const NetworkId &objectId, Archive::ReadIterator &data, bool isBaseline, int offset)
{
	if (isBaseline)
	{
 ...
 */
void SwgSnapshot::decodeMissionLocation  (Archive::ReadIterator &data, DB::BindableDouble &x,DB::BindableDouble &y, DB::BindableDouble &z, DB::BindableString<500> &planet, DB::BindableString<500> &region_name,DB::BindableNetworkId &cell) const
{
 ...
 * Helper function to avoid having to put too much logic into generated code.
 */
void SwgSnapshot::encodeMissionLocation  (Archive::ByteStream &data, const DB::BindableDouble &x,const DB::BindableDouble &y, const DB::BindableDouble &z, const DB::BindableString<500> &planet, const DB::BindableString<500> &region_name, const DB::BindableNetworkId &cell) const
{
	MissionLocation temp;
...
void SwgSnapshot::decodeVector(Archive::ReadIterator &data, DB::BindableDouble &x,DB::BindableDouble &y, DB::BindableDouble &z) const
{
	Vector temp;
	Archive::get(data,temp);
	x.setValue(temp.x);
...
void SwgSnapshot::encodeVector(Archive::ByteStream &data, const DB::BindableDouble &x, const DB::BindableDouble &y, const DB::BindableDouble &z) const
{
	Vector temp;
	x.getValue(temp.x);
	y.getValue(temp.y);
...
void SwgSnapshot::decodeLocationDataList(const NetworkId &objectId, size_t listId, Archive::ReadIterator &data, bool isBaseline)
{
	if (isBaseline)
	{
		size_t size, baselineCommandCount;
...
void SwgSnapshot::decodeLocation  (Archive::ReadIterator &data, DB::BindableDouble &x,DB::BindableDouble &y, DB::BindableDouble &z, DB::BindableNetworkId &cell, DB::BindableLong &planet) const
{
	Location temp;
	Archive::get(data,temp);
...
void SwgSnapshot::encodeLocation  (Archive::ByteStream &data, const DB::BindableDouble &x,const DB::BindableDouble &y, const DB::BindableDouble &z, const DB::BindableNetworkId &cell, const DB::BindableLong &planet) const
{
...
void SwgSnapshot::encodeQuests(Archive::ByteStream &data, 
	DB::BufferString const &part1, DB::BufferString const &part2,
	DB::BufferString const &part3, DB::BufferString const &part4) const
{
	std::string packedValue;
