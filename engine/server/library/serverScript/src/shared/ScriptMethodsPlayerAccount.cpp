...
jstring JNICALL ScriptMethodsPlayerAccountNamespace::getPlayerUsername(JNIEnv *env, jobject self, jlong player) {
    ...
    if (!JavaLibrary::getObject(player, creatureObject) || !creatureObject)
    {
        DEBUG_WARNING(true, ("JavaLibrary::getPlayerUsername: Unable to get CreatureObject for player [%d]", player));
        return nullptr;
    }
    ...
    if(creatureObject)
    {
        ...
        if(playerClient)
        {
            ...
            DEBUG_WARNING(true, ("JavaLibrary::getPlayerUsername: Unable to get Client for player [%d]", player));
            return nullptr;
        }
    }
}
